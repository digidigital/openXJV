...
#on ubuntu 23.04 package sudo apt install libxcb-cursor0
...
class UI(QMainWindow):
    ...
    def __downloadRequested(self, download):
        ...
        folder = QFileDialog.getExistingDirectory(None, "Speicherort wählen",
                                startFolder,
                                QFileDialog.Option.ShowDirsOnly | QFileDialog.Option.DontResolveSymlinks)
    ...
    def __reportPrintProgress(self, n):
        self.statusBar.showMessage(f"Druckfortschritt: {n}%")        
    
    def __printFinished(self):
        self.printLock=False
        self.statusBar.showMessage(f"Druckdatenverarbeitung abgeschlossen")  
    ...
    def __printRequested(self):
        ...
        try:
            ...
            self.worker = printPdfWorker(filepath)
            ...
            self.worker.progress.connect(self.__reportPrintProgress)
            # Start the thread
            self.thread.start()

            # Final resets
            self.thread.finished.connect(self.__printFinished)
            self.printLock=True          
            ...
            self.statusBar.showMessage('Der Druck ist fehlgeschlagen.')
            self.lastExceptionString=str(e)
            self.printLock=False 
            return    
