...
Migrated from helperScripts/Search.py to openxjv.utils module.
Supports extraction from PDF, DOCX, TXT, XML, XLSX, ODT, and ODS files.
"""
import os
import sys
...
def phrase_search(path = None, phrase = '', case_sensitive = False):
    """
    Searches for a phrase in a document.

    Args:
        path: Path to the document file
        phrase: Search phrase or list of words
    ...
    Raises:
        ...
        EOFError: If file contains no text
    """
...
def get_document_text(path = None, lower_case = False):
    ...
    Returns:
        str: Extracted text, or empty string if format not supported
    """
    if path.lower().endswith('.pdf'):
        text = PDF(path).text
    elif path.lower().endswith('.docx'):
